...
namespace LeapMotion
{
	public struct LeapInput
	{
		...
		public LeapInput(int id, float x, float y, float z)
		{
			Id = id;
			X = x;
			Y = y;
	...
	public class LeapHandler : IDisposable
	{
		...
		public void Dispose()
		{
			...
			_Controller.RemoveListener(_LeapListener);
			_Controller.Dispose();
		}

		/// <summary>
		/// Returns a state that maps the leap input to at max 2 inputs.
		/// </summary>
		public List<LeapInput> GetState()
		{
			var inputList = new List<LeapInput>();
			...
			if (!frame.Hands.IsEmpty)
			{
				...
				if (!fingers.IsEmpty)
				{
					...
					foreach (Finger finger in fingers)
					{
						...
						inputList.Add(new LeapInput(finger.Id, finger.TipPosition.x, finger.TipPosition.y, finger.TipPosition.z));
					}
					avgPos /= fingers.Count;
					//Debug.WriteLine("Hand has " + fingers.Count + " fingers, average finger tip position: " + avgPos);
				}

				//// Get the hand's sphere radius and palm position
				//Debug.WriteLine("Hand sphere radius: " + hand.SphereRadius.ToString("n2")
